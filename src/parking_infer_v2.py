"""
Parking spot detector – improved video inference (v2)
Based on user's provided implementation: lock-spots, tiling, calibration,
temporal smoothing and nicer visuals.
"""
import argparse
...
def add_args(parser):
    ...
    parser.add_argument("--spots", default=None,
                        help="Path to manual spots JSON to initialize locked spots (skips calibration)")
    parser.add_argument("--per-spot", action="store_true",
                        help="Run the classifier per spot crop (resizes each spot to imgsz). Improves sensitivity for small/rotated spots.")
    return parser
...
def run_per_spot(frame, model, spots, imgsz, conf, device):
    """Run model on each spot crop. Returns list of (spot_box, cls, conf)."""
    dets = []
    h, w = frame.shape[:2]
    for spot in spots:
        ...
        # resize crop to square imgsz for classifier
        try:
            crop_resized = cv2.resize(crop, (imgsz, imgsz), interpolation=cv2.INTER_LINEAR)
        except Exception:
            continue
        # run model on crop
        try:
            res = model(crop_resized, conf=conf, imgsz=imgsz, device=device, verbose=False)[0]
        except Exception:
            continue
        if len(res.boxes) == 0:
            continue
        ...
        cls = int(best.cls[0])
        cs = float(best.conf[0])
        dets.append((np.array([x1c, y1c, x2c, y2c], dtype=float), cls, cs))
    return dets


# ---------------------------------------------------------------------------
# Drawing (copied minimal helpers)
# ---------------------------------------------------------------------------
...
def run(args):
    ...
    # If manual spots provided, load and initialize locked spots immediately
    if args.spots:
        import json
        spots_json = Path(args.spots)
        if not spots_json.exists():
            raise SystemExit(f"spots JSON not found: {spots_json}")
        with open(spots_json, 'r', encoding='utf-8') as f:
            data = json.load(f)
        spots = [np.array([s['x1'], s['y1'], s['x2'], s['y2']], dtype=float) for s in data]
        spot_hits = [0] * len(spots)
        spot_cls_votes = [[] for _ in spots]
        spot_states = [None] * len(spots)
        spot_confs = [0.0] * len(spots)
        spot_last_seen = [-10**9] * len(spots)
        spot_history = [deque([None] * smooth_n, maxlen=smooth_n) for _ in spots]
        lock_spots = True
        calib_frames = 0
    ...
    def detect(frame, conf, imgsz, use_tile):
        if args.per_spot and spots:
            return run_per_spot(frame, model, spots, imgsz, conf, device)
        if use_tile:
...
def main():
    parser = argparse.ArgumentParser(description="Parking video inference v2")
    add_args(parser)
